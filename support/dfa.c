...
static unsigned int
newline_constraint (unsigned int constraint)
{
  return (constraint >> 6) & 7;
}
static unsigned int
letter_constraint (unsigned int constraint)
{
  return (constraint >> 3) & 7;
}
static unsigned int
other_constraint (unsigned int constraint)
{
  return constraint & 7;
}
...
static bool
prev_newline_dependent (unsigned int constraint)
{
  return ((constraint ^ (constraint >> 2)) & 0111) != 0;
}
static bool
prev_letter_dependent (unsigned int constraint)
{
...
prtok (token t)
{
  if (t <= END)
    ...
    {
      unsigned int ch = (unsigned int) t;
...
charclass_index (struct dfa *d, charclass const *s)
{
  ...
  for (i = 0; i < d->cindex; ++i)
    if (equal (s, &d->charclasses[i]))
      return i;
  d->charclasses =
    (charclass*) maybe_realloc (d->charclasses, d->cindex, &d->calloc,
                   ...
                   worry about that possibility.  */
                {
                  char const *cls
                    = (dfa->syntax.case_fold && (streq (str, "upper")
                                                 || streq (str, "lower"))
                       ? "alpha" : str);
                  const struct dfa_ctype *pred = find_pred (cls);
                  if (!pred)
      ...
      switch (c)
        ...
        case '$':
          ...
          if (dfa->syntax.syntax_bits & RE_CONTEXT_INDEP_ANCHORS
              || dfa->lex.left == 0
              || ((dfa->lex.left
                   > 0 + !(dfa->syntax.syntax_bits & RE_NO_BK_PARENS))
                  && (dfa->lex.ptr[!(dfa->syntax.syntax_bits & RE_NO_BK_PARENS)
                                   && (dfa->lex.ptr[0] == '\\')]
                      == ')'))
              || ((dfa->lex.left
                   > 0 + !(dfa->syntax.syntax_bits & RE_NO_BK_VBAR))
...
addtok_mb (struct dfa *dfa, token t, char mbprop)
{
  if (dfa->talloc == dfa->tindex)
    {
      dfa->tokens = (token*) xpalloc (dfa->tokens, &dfa->talloc, 1, -1,
                                      sizeof *dfa->tokens);
      if (dfa->localeinfo.multibyte)
        dfa->multibyte_prop
  ...
  if (dst->alloc < src->nelem)
    {
      free (dst->elems);
      dst->elems =
        (position*) xpalloc (NULL, &dst->alloc, src->nelem - dst->alloc, -1,
                             sizeof *dst->elems);
    }
  dst->nelem = src->nelem;
  if (src->nelem != 0)
...
insert (position p, position_set *s)
{
  ...
  s->elems = (position*) maybe_realloc (s->elems, count, &s->alloc, -1,
                                        sizeof *s->elems);
  ...
  if (m->alloc - s1->nelem < s2->nelem)
    {
      free (m->elems);
      m->alloc = s1->nelem;
      m->elems = (position*) xpalloc (NULL, &m->alloc, s2->nelem, -1,
                                      sizeof *m->elems);
    }
  m->nelem = 0;
...
{
  merge_constrained (s1, s2, (unsigned)-1, m);
}
...
static unsigned int
delete_pos (idx_t del, position_set *s)
{
  idx_t count = s->nelem;
  idx_t lo = 0, hi = count;
  while (lo < hi)
...
{
  unsigned int c = delete_pos (del, dst) & constraint;
...
state_index (struct dfa *d, position_set const *s, unsigned context)
{
  ...
  for (state_num j = 0; j < s->nelem; j++)
    {
      unsigned int c = d->constraints[s->elems[j].index];
  ...
  /* Create a new state.  */
  d->states = (dfa_state*) maybe_realloc (d->states, d->sindex, &d->salloc,
                                          -1, sizeof *d->states);
  d->states[i].hash = hash;
  alloc_position_set (&d->states[i].elems, s->nelem);
  copy (s, &d->states[i].elems);
  d->states[i].context = (unsigned char) context;
  d->states[i].constraint = (unsigned char) constraint;
          ...
          for (j = 0; j < nelem; j++)
            {
              ...
              if (flags[sindex] & OPT_REPEAT)
                delete_pos (sindex, &follows[sindex]);
...
{
  idx_t nleaves = 0;
  ptrdiff_t *map = (ptrdiff_t*) xnmalloc ((udx_t) d->tindex, sizeof *map);
  map[0] = nleaves++;
  for (idx_t i = 1; i < d->tindex; i++)
    map[i] = -1;

  token *tokens = (token*) xnmalloc ((udx_t) d->nleaves, sizeof *tokens);
  position_set *follows = (position_set*) xnmalloc ((udx_t) d->nleaves,
                                                    sizeof *follows);
  unsigned int *constraints = (unsigned int*) xnmalloc ((udx_t) d->nleaves,
                                                        sizeof *constraints);
  char *multibyte_prop =
    (d->localeinfo.multibyte
      ? (char*) xnmalloc ((udx_t) d->nleaves, sizeof *multibyte_prop)
      : (char*) NULL);
...
dfaoptimize (struct dfa *d)
{
  ...
  d->constraints = (unsigned int*) xcalloc ((udx_t) d->tindex,
                                            sizeof *d->constraints);
...
dfaanalyze (struct dfa *d, bool searchflag)
{
  ...
  alloc_position_set (&merged, d->nleaves);
  d->follows = (position_set*) xcalloc ((udx_t) tindex, sizeof *d->follows);
  position_set *backward
    = d->epsilon ? (position_set*) xcalloc ((udx_t) tindex, sizeof *backward)
                 : (position_set*) NULL;
  ...
  for (idx_t i = 0; i < tindex; i++)
    {
      ...
      prtok (d->tokens[i]);
      putc ('\n', stderr);
      fputs (stk[-1].nullable ? " nullable: yes\n" : " nullable: no\n",
             stderr);
  ...
  d->separates = (unsigned*) xcalloc ((udx_t) tindex, sizeof *d->separates);
  ...
  d->initstate_notbol = state_index (d, &tmp, separate_contexts ^ CTX_ANY);
  d->min_trcount = (int) d->initstate_notbol;
  if (separate_contexts & CTX_LETTER)
    d->min_trcount = (int) state_index (d, &tmp, CTX_LETTER);
  d->min_trcount++;
  d->trcount = 0;
  ...
  if (oldalloc < d->sindex)
    {
      ...
      realtrans = (state_num**) xpalloc (realtrans, &newalloc1,
                                         d->sindex - oldalloc,
                                         -1, sizeof *realtrans);
...
build_state (state_num s, struct dfa *d, unsigned char uc)
{
  ...
  if (!trans)
    {
      ...
      d->trcount++;
      *ptrans = trans = (state_num*) xmalloc (NOTCHAR * sizeof *trans);
  ...
  for (int i = 0; i < NOTCHAR; i++)
    if (tstbit ((unsigned int) i, &label))
  ...
  if (! d->mb_trans[s])
    {
      ...
      d->mb_trans[s] = (state_num*) xmalloc (TRANSALLOC_SIZE);
...
static char *
icatalloc (char *old_c, char const *new_c)
{
  idx_t newsize = (idx_t) strlen (new_c);
  if (newsize == 0)
    return old_c;
...
{
  char **cpp = (char**) xzalloc (sizeof *cpp);
...
static char **
addlists (char **old_c, char **new_c)
{
  for (; *new_c; new_c++)
    old_c = enlist (old_c, *new_c, (idx_t) strlen (*new_c));
  return old_c;
}
...
{
  char **both = (char**) xzalloc (sizeof *both);
      ...
      switch (t)
        ...
        case OR:
          {
            ...
            lmp->left[i] = '\0';
            /* Right side */
            ln = (idx_t) strlen (lmp->right);
            rn = (idx_t) strlen (rmp->right);
            n = ln;
            if (n > rn)
              n = rn;
            for (i = 0; i < n; ++i)
        ...
        default:
          ...
          mp->is[0] = mp->left[0] = mp->right[0]
            = (char) (case_fold_unibyte ? toupper ((int) t) : (int) t);
...
/* Initialize DFA.  */
void
dfasyntax (struct dfa *d, struct localeinfo const *linfo,
           reg_syntax_t bits, int dfaopts)
{
  memset (d, 0, offsetof (struct dfa, dfaexec));
  d->dfaexec = linfo->multibyte ? dfaexec_mb : dfaexec_sb;
  d->localeinfo = *linfo;

  d->fast = !d->localeinfo.multibyte;

  d->canychar = -1;
  d->syntax.syntax_bits_set = true;
  d->syntax.case_fold = (bits & RE_ICASE) != 0;
  d->syntax.anchor = (dfaopts & DFA_ANCHOR) != 0;
  d->syntax.eolbyte = (unsigned char) (dfaopts & DFA_EOL_NUL ? '\0' : '\n');
  d->syntax.syntax_bits = bits;

  for (int i = CHAR_MIN; i <= CHAR_MAX; ++i)
    {
      unsigned char uc = (unsigned char) (char) i;

      d->syntax.sbit[uc] = (char) char_context (d, uc);
      switch (d->syntax.sbit[uc])
        {
        case CTX_LETTER:
          setbit (uc, &d->syntax.letters);
          break;
        case CTX_NEWLINE:
