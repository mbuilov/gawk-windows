...
#include "oldnames.h"
#define execvp(file, args)	w32_execvp(file, args)
#endif
...
typedef struct break_point {
	...
	awk_ulong_t ignore_count;
	awk_ulong_t hit_count;
	const char *src;
...
#define BP_TEMP         4
#define BP_IGNORE       8
#define BP_FLAGS_MASK	(BP_ENABLE|BP_ENABLE_ONCE|BP_TEMP|BP_IGNORE)

} BREAKPOINT;

static BREAKPOINT breakpoints = { &breakpoints, &breakpoints, 0, 0u, 0u, NULL, NULL,
				  { NULL, NULL, D_illegal, NULL, NULL },
...
#define cur_size  value[0].sz
#define old_value value[1].n
#define old_size  value[1].sz
...
#define LI_FLAGS_MASK	(PARAM|SUBSCRIPT|FIELD_NUM|OLD_IS_ARRAY|CUR_IS_ARRAY)
};

#define IS_PARAM(d)	(((d)->li_flags & PARAM) != 0)
#define IS_SUBSCRIPT(d)	(((d)->li_flags & SUBSCRIPT) != 0)
#define IS_FIELD(d)	(((d)->li_flags & FIELD_NUM) != 0)
#define WATCHING_ARRAY(d)	(((d)->li_flags & CUR_IS_ARRAY) != 0)

static struct list_item display_list = { &display_list, &display_list, 0,
	0, NULL, NULL, NULL, 0ul, {NULL, NULL, D_illegal, NULL, NULL}, {NULL, NULL, NULL}, {{NULL}, {NULL}}, false, 0 };
...
static struct {
	...
	const char *source;       /* source name last time we stopped execution,
	                             used by 'next', 'until' and 'step' commands */
	unsigned sourceline;      /* source line number last time we stopped execution,
	                             used by 'next', 'until' and 'step' commands */
	...
	bool (*check_func)(INSTRUCTION **);     /* function to decide when to suspend
...
static bool do_save_history = true;
static bool do_save_options = true;
...
unsigned pager_quit_tag_valid = 0;
...
static fd_t open_readfd(const char *file);
...
static bool execute_commands(struct commands_item *commands);
static void delete_commands_item(struct commands_item *c);
static NODE *execute_code(volatile INSTRUCTION *code);
static bool pre_execute_code(INSTRUCTION **pi);
static bool parse_condition(enum argtype type, unsigned num, char *expr);
static BREAKPOINT *add_breakpoint(INSTRUCTION *prevp, INSTRUCTION *ip, const char *src, bool silent);
...
static struct list_item *add_item(struct list_item *list, enum argtype type, NODE *symbol, const char *pname);
...
static void print_instruction(INSTRUCTION *pc, Func_print print_func, FILE *fp, bool in_dump);
static void print_ns_list(INSTRUCTION *pc, Func_print print_func, FILE *fp, bool in_dump);
static int print_code(INSTRUCTION *pc, void *x);
static void next_command(void);
static void debug_post_execute(INSTRUCTION *pc);
static bool debug_pre_execute(INSTRUCTION **pi);
static char *g_readline(const char *prompt);
static bool prompt_yes_no(const char *mesg, char res_true, bool res_default, FILE *fp);
...
static inline void
push_binding(volatile jmp_buf *stack_place, jmp_buf *tag, unsigned *valid)
{
	if ((*valid)++)
		memcpy(volatile_cast(stack_place), tag, sizeof(*tag));
}

static inline void
pop_binding(volatile jmp_buf *stack_place, jmp_buf *tag, unsigned *valid)
{
	if (--(*valid))
		memcpy(tag, volatile_cast(stack_place), sizeof(*tag));
}
	...
	while ((n = read(s->fd, buf, to_read)) > 0) {
		...
		while (p < end) {
			if (*p++ == '\n') {
				...
				{
					size_t line_len = (size_t) (p - buf);
					if (line_len > (unsigned)-1 - ofs)
						goto too_big;
					*++pos = ofs + (unsigned) line_len;
				}
				if ((pos[0] - pos[-1]) > maxlen)
					maxlen = pos[0] - pos[-1];	/* length including NEWLINE */
			}
		}
		if ((size_t) n >= (unsigned)-1 - ofs)
			goto too_big;
		ofs += (unsigned) n;
	}
	efree(buf);
...
too_big:
	efree(buf);
	d_error(_("source file `%s' is too big"), s->src);
	return -1;
}
...
static unsigned
print_lines(const char *src, unsigned start_line, unsigned nlines)
{
	SRCFILE *s;
	unsigned *pos;
	unsigned i;
	...
	if (nlines - 1 > s->srclines - start_line)
...
{
	unsigned line_first, line_last;
	unsigned count = list_size;
	INSTRUCTION *rp;
	const char *src = cur_srcfile->src;
	(void) cmd;
...
line:
			if (arg->a_int <= (int) (list_size / 2))
				line_first = 1;
			else if ((awk_ulong_t) arg->a_int < (unsigned)-1 - list_size / 2)
				line_first = (unsigned) arg->a_int - list_size / 2;
			else
				line_first = (unsigned)-1 - list_size;
		}
		break;
...
range:
		if (arg->a_int <= 0)
			line_first = 1;
		else if ((awk_ulong_t) arg->a_int < (unsigned)-1)
			line_first = (unsigned) arg->a_int;
		else
			line_first = (unsigned)-1 - 1;
		arg = arg->next;
		assert(arg != NULL);
		assert(arg->type == D_int);
		if (arg->a_int > 0 && (awk_ulong_t) arg->a_int >= line_first &&
			(awk_ulong_t) arg->a_int < (unsigned)-1)
		{
			count = (unsigned) arg->a_int - line_first + 1;
		}
		else if (count > (unsigned)-1 - line_first)
			count = (unsigned)-1 - line_first;
		break;
	...
	case A_BREAK:
		...
		if (setjmp(pager_quit_tag) == 0) {
			...
			for (b = breakpoints.prev; b != &breakpoints; b = b->prev) {
				...
				if (b->hit_count)
					gprintf(out_fp, _("\tnumber of hits = %" AWKULONGFMT "\n"), TO_AWK_ULONG(b->hit_count));
				if ((b->flags & BP_IGNORE) != 0)
					gprintf(out_fp, _("\tignore next %" AWKULONGFMT " hit(s)\n"), TO_AWK_ULONG(b->ignore_count));
				if (b->cndn.code != NULL)
			...
			for (d = list->prev; d != list; d = d->prev) {
				...
				if (IS_SUBSCRIPT(d)) {
					...
					for (i = 0; i < d->num_subs; i++) {
						NODE *sub;
						sub = d->subs[i];
						gprintf(out_fp, "[\"%.*s\"]", TO_PRINTF_WIDTH(sub->stlen), sub->stptr);
					}
					gprintf(out_fp, "\n");
				} else if (IS_FIELD(d))
...
static
NODE *find_symbol(const char *name, const char **pname)
{
	NODE *r = NULL;
...
static int
print_array(volatile NODE *arr, const char *arr_name)
{
	NODE *subs;
	NODE **list;
	...
	if (assoc_empty((NODE *) volatile_cast(arr))) {
		gprintf(out_fp, _("array `%s' is empty\n"), arr_name);
		return 0;
	}

	num_elems = assoc_length((NODE *) volatile_cast(arr));

	/* sort indices, sub_arrays are also sorted! */
	list = assoc_list((NODE *) volatile_cast(arr), "@ind_str_asc", SORTED_IN);
	...
	if (setjmp(pager_quit_tag) == 0) {
		for (i = 0u; ret == 0 && i < num_elems; i++) {
			...
			if (r->type == Node_var_array)
				...
				gprintf(out_fp, "%s[\"%.*s\"] = ", arr_name, TO_PRINTF_WIDTH(subs->stlen), subs->stptr);
				valinfo((NODE *) volatile_cast(r), gprintf, out_fp);
			}
		}
	} else
		ret = 1;
...
static void
print_subscript(NODE *arr, const char *arr_name, CMDARG *a, unsigned count)
{
	NODE *r, *subs;
	...
	if (r == NULL)
		...
		fprintf(out_fp, "%s[\"%.*s\"] = ", arr_name, TO_PRINTF_WIDTH(subs->stlen), subs->stptr);
				...
				for (; count > 0; count--) {
					...
					if (value == NULL) {
						...
						fprintf(out_fp, _("`%s[\"%.*s\"]' is not an array\n"),
									name, TO_PRINTF_WIDTH(subs->stlen), subs->stptr);
						break;
					} else {
						r = value;
						name = r->vname;
		...
		for (; count > 0; count--) {
			...
			if (count == 1) {
				if (value != NULL && value->type == Node_var_array)
					...
					fprintf(out_fp, "%s[\"%.*s\"] = ", name, TO_PRINTF_WIDTH(subs->stlen), subs->stptr);
...
delete_item(struct list_item *d)
{
	...
	if ((d->li_flags & CUR_IS_ARRAY) == 0)
		unref(d->cur_value);
	if ((d->li_flags & OLD_IS_ARRAY) == 0)
		unref(d->old_value);
...
static struct list_item *
add_item(struct list_item *list, enum argtype type, NODE *symbol, const char *pname)
{
	struct list_item *d;
	...
	if (type == D_field) {
		/* field number */
		d->symbol = symbol;
		d->li_flags |= FIELD_NUM;
	} else if (type == D_subscript) {
		/* subscript */
		d->symbol = symbol;
		d->li_flags |= SUBSCRIPT;
	} else {
		/* array or variable */
		d->symbol = symbol;
	}
...
do_add_item(struct list_item *list, CMDARG *arg)
{
	...
	if (list == &watch_list) {
		...
		if (item != NULL && arg != NULL) {
			if (parse_condition(D_watch, item->number, arg->a_string))
...
do_delete_item(struct list_item *list, CMDARG *arg)
{
	...
	for (; arg != NULL; arg = arg->next) {
		...
		if (arg->type == D_range) {
			...
			if (i <= 0)
				i = 1;
			if (j >= 0 && (awk_ulong_t) j > list->number)
				j = (awk_long_t) list->number;
			for (i--; i < j; i++) {
				if ((d = find_item(list, (unsigned) i + 1)) != NULL)
					delete_item(d);
			}
		} else {
			assert(arg->type == D_int);
			if ((d = find_item(list, (unsigned) arg->a_int)) == NULL) {
				...
				if (list == &display_list)
					d_error(_("no display item numbered %" AWKLONGFMT ""),
						TO_AWK_LONG(arg->a_int));
				else
					d_error(_("no watch item numbered %" AWKLONGFMT ""),
						TO_AWK_LONG(arg->a_int));
			} else
				delete_item(d);
		}
		...
		for (i = 0; i < count; i++) {
			...
			if (r->type == Node_var_array) {
				...
				fprintf(out_fp, "%u: %s[\"%.*s\"] = ", d->number,
							d->sname, TO_PRINTF_WIDTH(sub->stlen), sub->stptr);
				valinfo(r, fprintf, out_fp);
			}
		}
	} else if (IS_FIELD(d)) {
...
cmp_val(struct list_item *w, NODE *old_v, NODE *new_v)
{
	...
	if (new_v->type == Node_var_array)	/* 5 */
		return true;
	return 0 != cmp_nodes(old_v, new_v, true);	/* 4 */
}
...
watchpoint_triggered(struct list_item *w)
{
	...
	if ((w->li_flags & OLD_IS_ARRAY) == 0)
		unref(w->old_value);
	w->li_flags &= ~OLD_IS_ARRAY;
	if (WATCHING_ARRAY(w)) {	/* 7, 8, 9 */
		w->old_size = w->cur_size;
		w->li_flags |= OLD_IS_ARRAY;
		if (! t2) {
			w->li_flags &= ~CUR_IS_ARRAY;
			w->cur_value = 0;
...
do_watch(CMDARG *arg, enum argtype cmd)
{
	...
	if (IS_SUBSCRIPT(w)) {
		...
		for (i = 0; i < w->num_subs; i++) {
			sub = w->subs[i];
			fprintf(out_fp, "[\"%.*s\"]", TO_PRINTF_WIDTH(sub->stlen), sub->stptr);
		}
		fprintf(out_fp, "\n");
	} else if (IS_FIELD(w))
...
#define print_func fprintf
	...
	print_func(fp, ")");
	if (defn)
		print_func(fp, _(" in file `%s', line %u\n"),
...
static void
print_frame(NODE *func, const char *src, unsigned srcline)
{
	if (func == NULL)
		fprintf(out_fp, "main()");
	else {
...
do_frame(CMDARG *arg, enum argtype cmd)
{
	...
	if (arg && arg->type == D_int) {
		if (arg->a_int < 0 || (awk_ulong_t) arg->a_int > fcall_count) {
...
static INSTRUCTION *
find_rule(const char *src, unsigned lineno)
{
	INSTRUCTION *rp;
...
static INSTRUCTION *
mk_breakpoint(const char *src, unsigned srcline)
{
	INSTRUCTION *bp;
	BREAKPOINT *b;
...
static BREAKPOINT *
add_breakpoint(INSTRUCTION *prevp, INSTRUCTION *ip, const char *src, bool silent)
{
	BREAKPOINT *b;
	INSTRUCTION *bp;
	...
	while (ip->opcode == Op_breakpoint && ip->source_line == lineno) {
		if (! silent) {
			...
			if ((b->flags & BP_ENABLE) != 0) {
				if ((b->flags & BP_IGNORE) != 0)
					fprintf(out_fp,
			_("Note: breakpoint %u (enabled, ignore next %" AWKULONGFMT " hits), also set at %s:%u"),
						b->number,
						TO_AWK_ULONG(b->ignore_count),
						b->src,
						lineno);
			...
			_("Note: breakpoint %u (enabled), also set at %s:%u"),
				...
				if ((b->flags & BP_IGNORE) != 0)
					fprintf(out_fp,
			_("Note: breakpoint %u (disabled, ignore next %" AWKULONGFMT " hits), also set at %s:%u"),
						b->number,
						TO_AWK_ULONG(b->ignore_count),
						b->src,
						lineno);
...
*/
	...
	case D_int:		/* break lineno */
		ln = arg->a_int;
		if (ln <= 0 || (awk_ulong_t) ln > s->srclines)
			d_error(_("line number %" AWKLONGFMT " in file `%s' is out of range"), TO_AWK_LONG(ln), src);
		else {
			lineno = (unsigned) ln;
...
func:
	...
	arg = arg->next;
	if (b != NULL && arg != NULL) {
		if (parse_condition(D_break, b->number, arg->a_string))
			arg->a_string = NULL;	/* don't let free_cmdarg free it */
		else
...
do_clear(CMDARG *arg, enum argtype cmd)
{
	...
	case D_int:	/* clear lineno */
		ln = arg->a_int;
		if (ln <= 0 || (awk_ulong_t) ln > s->srclines) {
			d_error(_("line number %" AWKLONGFMT " in file `%s' out of range"), TO_AWK_LONG(ln), src);
			return false;
		}
		lineno = (unsigned) ln;
		break;
...
do_enable_breakpoint(CMDARG *arg, enum argtype cmd)
{
	...
	for (; arg != NULL; arg = arg->next) {
		if (arg->type == D_range) {
			...
			if (i <= 0)
				i = 1;
			if (j >= 0 && (awk_ulong_t) j > breakpoints.number)
				j = (awk_long_t) breakpoints.number;
			for (i--; i < j; i++) {
				if ((b = find_breakpoint((unsigned) i + 1)) != NULL)
					enable_breakpoint(b, disp);
			}
		} else {
			assert(arg->type == D_int);
...
do_ignore_breakpoint(CMDARG *arg, enum argtype cmd)
{
	...
	if ((b = find_breakpoint((unsigned) arg->a_int)) == NULL)
		...
		b->ignore_count = (awk_ulong_t) arg->next->a_int;
		if (b->ignore_count) {
			b->flags |= BP_IGNORE;
			fprintf(out_fp, _("Will ignore next %" AWKULONGFMT " crossing(s) of breakpoint %u.\n"),
					TO_AWK_ULONG(b->ignore_count), b->number);
		} else {
			b->flags &= ~BP_IGNORE;
...
do_continue(CMDARG *arg, enum argtype cmd)
{
	...
	if (!arg->a_int) {
		fprintf(out_fp, _("Ignoring zero argument value.\n"));
		return true;
	}
	b->flags |= BP_IGNORE;
	b->ignore_count = (awk_ulong_t) arg->a_int;
	fprintf(out_fp, _("Will ignore next %" AWKULONGFMT " crossings of breakpoint %u.\n"),
				TO_AWK_ULONG(b->ignore_count), stop.break_point);
	return true;
}
...
do_until(CMDARG *arg, enum argtype cmd)
{
	...
	case D_int:	/* until lineno */
		ln = arg->a_int;
		if (ln <= 0 || (awk_ulong_t) ln > s->srclines) {
			d_error(_("line number %" AWKLONGFMT " in file `%s' out of range"),
						TO_AWK_LONG(ln), src);
			return false;
		}
		lineno = (unsigned) ln;
...
#define print_value(X, S, V) do {                                       \
	...
	print_value((w->li_flags & OLD_IS_ARRAY) != 0, old_size, old_value);
	fprintf(out_fp, "  New value: ");
	print_value((w->li_flags & CUR_IS_ARRAY) != 0, cur_size, cur_value);
...
static void
print_ns_list(INSTRUCTION *pc, Func_print print_func, FILE *fp, bool in_dump)
{
	for (; pc != NULL; pc = pc->nexti) {
...
do_dump_instructions(CMDARG *arg, enum argtype cmd)
{
	...
	if (arg != NULL && arg->type == D_string) {
		...
		(void) foreach_func(funcs, print_code, &pf_data);
		efree(funcs);
		fclose(fp);
		return false;
	}
...
static size_t
serialize_subscript(char *buf, size_t buflen, const struct list_item *item)
{
	int nchar;
	unsigned i;
	size_t bl;
...
serialize_list(enum env_type type)
{
	...
	static size_t buflen = 0;
	size_t bl;
	const BREAKPOINT *b = NULL;
	const struct list_item *wd = NULL;
	HIST_ENTRY **hist_list = NULL;
	unsigned hist_index = 0;
	const struct dbg_option *opt = NULL;
	const struct commands_item *commands = NULL, *c;
	unsigned cnum = 0;
	const struct condition *cndn = NULL;
...
#define SERIALIZE_BUFSIZ 512
	...
	if (buf == NULL) {	/* first time */
		...
		emalloc(buf, char *, buflen + 1/*'\0'*/, "serialize");
	}
	bl = 0;
...
enlarge_buffer:
			buflen *= 2;
			erealloc(buf, char *, buflen + 1/*'\0'*/, "serialize");
		}
...
#undef SERIALIZE_BUFSIZ
		...
		case BREAK:
			...
			sz = (unsigned) snprintf(buf + bl, buflen - bl,
			                 "%s%c%u%c%d%c%" AWKULONGFMT "%c%" AWKULONGFMT "%c%u%c",
			                 b->src, FSEP, b->bpi->source_line, FSEP, b->flags, FSEP,
			                 TO_AWK_ULONG(b->ignore_count), FSEP,
			                 TO_AWK_ULONG(b->hit_count), FSEP, b->number, FSEP);
			cnum = b->number;
			commands = &b->commands;
			cndn = &b->cndn;
			break;
		...
		case HISTORY:
		{
			...
			break;
		case OPTION:
			opt = (const struct dbg_option *) ptr;
			if (opt->str_val != NULL)
				sz = (unsigned) snprintf(buf + bl, buflen - bl,
								"%s%c%s%c", opt->name, FSEP, *(opt->str_val), FSEP);
			else if (opt->flag_val != NULL)
				sz = (unsigned) snprintf(buf + bl, buflen - bl,
								"%s%c%d%c", opt->name, FSEP, (int) *(opt->flag_val), FSEP);
			else
				sz = (unsigned) snprintf(buf + bl, buflen - bl,
		...
		case WATCH:
			...
			ptr = (type == BREAK) ? (const void *) b->prev : (const void *) wd->prev;
			break;
		case DISPLAY:
			ptr = wd->prev;
			break;
		case HISTORY:
			ptr = hist_list[++hist_index];
			break;
		case OPTION:
			ptr = ++opt;
			break;
...
static void
unserialize_commands(const char *str, size_t str_len)
{
...
static bool
unserialize_awkulong(const char *p, const size_t len, awk_ulong_t *aul, awk_ulong_t lim)
{
...
unserialize_list_item(struct list_item *list, const char *const pstr[], const size_t pstr_len[], unsigned field_cnt)
{
	...
	if (!unserialize_awkulong(pstr[0], pstr_len[0], &aul, (unsigned)-1))
		return NULL;
	num = (unsigned) aul;

	if (!unserialize_awkulong(pstr[1], pstr_len[1], &aul, (unsigned)-1/2))
		return NULL;
	type = (enum argtype) (int) aul;
	...
	if (type == D_field) {
		...
		cnt = 3;
		if (type == D_subscript) {
			if (!unserialize_awkulong(pstr[3], pstr_len[3], &aul, (unsigned)-1))
				return NULL;
			sub_cnt = (unsigned) aul;
...
static BREAKPOINT *
unserialize_breakpoint(const char *const pstr[], const size_t pstr_len[], unsigned field_cnt)
{
	char *src;
	BREAKPOINT *b = NULL;
	INSTRUCTION *rp;
	SRCFILE *s;
	awk_ulong_t lineno, flags, ignore_count, hit_count, number;
	...
	if (!unserialize_awkulong(pstr[1], pstr_len[1], &lineno, s->srclines))
		return NULL;
	rp = find_rule(s->src, (unsigned) lineno);
	if (rp == NULL)
		return NULL;

	if (!unserialize_awkulong(pstr[2], pstr_len[2], &flags, (awk_ulong_t)BP_FLAGS_MASK))
		return NULL;
	if (!unserialize_awkulong(pstr[3], pstr_len[3], &ignore_count, (awk_ulong_t)-1))
		return NULL;
	if (!unserialize_awkulong(pstr[4], pstr_len[4], &hit_count, (awk_ulong_t)-1))
		return NULL;
	if (!unserialize_awkulong(pstr[5], pstr_len[5], &number, (unsigned)-1))
		return NULL;

	if ((b = set_breakpoint_at(rp, (unsigned) lineno, true)) == NULL)
		return NULL;

	b->flags = (int) flags;
	b->ignore_count = ignore_count;
	b->hit_count = hit_count;
	b->number = (unsigned) number;	/* same number as previous run */
	...
	if (field_cnt > 7) {	/* condition expression */
		...
		if (!parse_condition(D_break, b->number, expr))
			efree(expr);
	}
...
{
	const char *val, *p, *q, *r, *s;
#define MAX_FIELD 30
	const char *pstr[MAX_FIELD];
	size_t pstr_len[MAX_FIELD];
...
static bool
prompt_yes_no(const char *mesg, char res_true, bool res_default, FILE *fp)
{
	char *in_str;
	bool ret = res_default; /* default */
...
execute_code(volatile INSTRUCTION *code)
{
	...
	push_binding(&fatal_tag_stack, &fatal_tag, &fatal_tag_valid);
	if (setjmp(fatal_tag) == 0) {
		(void) interpret((INSTRUCTION*) volatile_cast(code));
		r = POP_SCALAR();
	} else	/* fatal error */
...
do_condition(CMDARG *arg, enum argtype cmd)
{
	...
	if (arg != NULL)
		expr = arg->a_string;
	if (parse_condition(type, num, expr) && arg != NULL)
		arg->a_string = NULL;	/* don't let free_cmdarg free it */
	return false;
}
...
{
	const struct command_source *cs;
	for (cs = cmd_src; cs != NULL; cs = cs->next) {
